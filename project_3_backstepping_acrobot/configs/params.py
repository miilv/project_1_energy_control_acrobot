...
class ActuatorParams:
    """First-order actuator dynamics  Ta * dtau2 = u - tau2."""

    Ta: float = 0.005  # actuator time constant [s]
...
class ControlParams:
    ...
    kz: float = 25.0    # backstepping gain on the torque-tracking error z = tau2 - tau2*
    u_max: float = 50.0  # command saturation limit [N*m]
    switch_threshold: float = 0.04  # weighted state-error norm for LQR activation
    Q_diag: tuple = (10.0, 10.0, 1.0, 1.0)  # LQR state cost diagonal (mechanical 4D, P1 weights)
    R_val: float = 5.0  # LQR input cost (larger -> less aggressive gains)
    k_tau: float = 1.0  # extra LQR feedback gain on the actuator state tau2
...
class SimParams:
    ...
    x0: tuple = (-1.4, 0.001, 0.0, 0.0, 0.0)  # initial state [q1, q2, dq1, dq2, tau2]
    # The 0.001 rad offset on q2 breaks the spurious LaSalle invariant manifold
    # (Project 2 §3.6) — without it the trajectory stays locked near q2 = 0.
